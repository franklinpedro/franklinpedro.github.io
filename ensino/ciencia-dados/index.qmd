...
-   [**ciência de dados em r.**](https://drive.google.com/file/d/13I5p76MWiLni-qEtZUJ7ubqNRBSrxD-H/view?usp=sharing). Livro escrito pela incrível equipe do [curso-r](https://curso-r.com/).
...
dados <- read_sheet("https://docs.google.com/spreadsheets/d/1JyNIvLwUXczt1ub69jXYCjOb0Sm6LrEfxPCzn-evMBo/edit?usp=sharing")
```
...
dados |>
  ...
  mutate(
    ...
    data_formatada = glue("{nome_dia}, <br>{day(data)} de {nome_mes}")
  ) |>
  ...
  fmt_markdown(columns = c(data_formatada, conteudo, materiais, referencias)) |>
  cols_width(
    data_formatada ~ px(200),
    conteudo ~ px(400),
    materiais ~ px(500),
    referencias ~ px(300)
  ) |>
